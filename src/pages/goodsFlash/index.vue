 ...
 * @Author: your name
 * @Date: 2019-08-23 15:02:14
 * @LastEditTime: 2019-08-23 15:02:14
...
<template>
  <div class="goods">
    ...
    <div class="price-bg">
      ...
      <p class="salePrice" >
        ￥{{allData.offerCommand ? allData.offerCommand.offerSkuList[0].discountedPrice : ''}}
        <span class="tag">限时抢购价</span>
      </p>
      <div class="countDown" v-if="allData.offerCommand">
        <div v-if="timeFlag == false">
          <p class="title">距限时抢购活动{{saleType}}仅剩</p>
          <div class="time">
            <span>{{day}}</span>天
            <span>{{h}}</span>:
            <span>{{m}}</span>:
        ...
        <p class="title" v-else>限时抢购活动已结束</p>
      </div>
    </div>
    ...
    <div class="bottom-fixed">
      ...
      <div class="btn-group">
        <div class="buyNow" v-if="saleType == '结束' && timeFlag== false && goodsInfo.realQuantity > 0" @click="openSku('buyNow')">立即抢购</div>
        <div class="buyNow" v-else @click="gotoOld(goodsInfo)">查看原商品</div>
      </div>
    </div>
...
import { detailGoodsFlash } from '../../api/goodsFlash';
...
export default {
  onShow() {
    ...
    this.code = this.$root.$mp.query.itemCode;
  ...
  methods: {
    ...
    countTime: function() {
      ...
      //设置截止时间
      let end = Number(this.allData.offerExpireTime);
      //设置开始时间
      let start = Number(this.allData.offerAnotherTime);
